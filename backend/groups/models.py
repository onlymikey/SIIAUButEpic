...
class Group(models.Model):
    ...
    def clean(self):
        ...
        if not self.study_period or len(self.study_period) != 5:
            raise ValidationError("El formato del periodo de estudio debe ser YYYYA, donde A es una letra del calendario.")

        try:
            year = int(self.study_period[:4])
        except ValueError:
            raise ValidationError("Los primeros 4 caracteres del periodo de estudio deben ser un año válido.")

        current_year = datetime.now().year
        if year < current_year:
            raise ValidationError("El año del periodo de estudio no puede ser menor al año actual.")
        calendar_letter = self.study_period[4]
        if not calendar_letter.isalpha():
            raise ValidationError("El formato del periodo de estudio debe terminar en una letra del calendario (e.g., 2024B).")

        # Validar que el periodo de estudio no sea menor al año de la fecha de inicio
        if start_date_obj and year < start_date_obj.year:
            raise ValidationError("El periodo de estudio no puede ser menor al año especificado en la fecha de inicio.")
